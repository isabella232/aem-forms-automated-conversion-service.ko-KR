---
title: 변환된 적응형 양식을 JSON 스키마로 데이터베이스에 제출
description: 변환된 적응형 양식을 JSON 스키마와 함께 데이터베이스에 제출하려면 양식 데이터 모델을 생성하고 AEM 워크플로우에서 이를 참조합니다.
...
automated forms conversion 서비스를 사용하면 비대화형 PDF 양식, Acro Form 또는 XFA 기반 PDF 양식을 적응형 양식으로 변환할 수 있습니다. 전환 프로세스를 시작하는 동안 데이터 바인딩이 있거나 없는 응용 양식을 생성하는 옵션이 있습니다.

데이터 바인딩 없이 적응형 양식을 생성하도록 선택하는 경우 변환 후 변환된 적응형 양식을 양식 데이터 모델, XML 스키마 또는 JSON 스키마에 통합할 수 있습니다. 양식 데이터 모델의 경우 적응형 양식 필드를 양식 데이터 모델로 수동으로 바인딩해야 합니다. 그러나 데이터 바인딩으로 응용 양식을 생성하는 경우 전환 서비스는 적응형 양식을 JSON 스키마와 자동으로 연결하고 적응형 양식과 JSON 스키마에서 사용 가능한 필드 간에 데이터 바인딩을 생성합니다. 그런 다음 적응형 양식을 원하는 데이터베이스와 통합하고, 양식의 데이터를 채우고, 데이터베이스에 제출할 수 있습니다. 마찬가지로, 데이터베이스와의 통합이 성공한 후 변환된 응용 양식의 필드를 구성하여 데이터베이스에서 값을 검색하고 응용 양식 필드를 미리 채울 수 있습니다.

다음 그림은 변환된 응용 양식을 데이터베이스와 통합하는 다른 단계를 설명합니다.
...
* AEM 인스턴스용 [최신 서비스 팩](https://helpx.adobe.com/kr/experience-manager/aem-releases-updates.html)을(를) 설치합니다.
...
변환된 적응형 양식을 AEM 워크플로우를 사용하여 데이터베이스와 통합하기 위한 사용 사례를 실행하려면 다음 샘플 PDF 파일을 다운로드하십시오.

다음을 사용하여 문의 양식 샘플을 다운로드할 수 있습니다.

[파일 가져오기](assets/sample_contact_us_form.pdf)

PDF 파일은 Automated forms conversion 서비스에 대한 입력 역할을 합니다. 서비스는 이 파일을 응용 양식으로 변환합니다. 다음 이미지는 샘플 연락처 양식을 PDF 형식으로 보여 줍니다.
...
이 자습서에서는 MySQL 데이터베이스를 소스로 사용하여 양식 데이터 모델을 만듭니다. 데이터베이스에 스키마를 만들고 적응형 형식으로 사용할 수 있는 필드를 기반으로 스키마에 **contacuity** 테이블을 추가합니다.
...
1. 찾기 및 클릭하여 웹 콘솔 구성에서 편집 모드에서 **[!UICONTROL Apache Sling Connection Pooled DataSource]**&#x200B;을(를) 엽니다. 다음 표에 설명된 대로 속성의 값을 지정합니다.
...
MYSQL을 데이터 소스로 구성한 후에는 다음 단계를 실행하여 양식 데이터 모델을 생성합니다.
...
1. **[!UICONTROL Edit]**&#x200B;을(를) 누르고 왼쪽 창에 나열된 데이터 소스를 확장하여 **contacts** table, **[!UICONTROL get]** 및 **[!UICONTROL insert]** 서비스를 선택하고 **[!UICONTROL Add Selected]**&#x200B;를 누릅니다.

   ![샘플 데이터 mysql](assets/fdm_details_workfdlow_submit.png)

1. 오른쪽 창에서 데이터 모델 개체를 선택하고 **[!UICONTROL Edit Properties]**&#x200B;을 누릅니다. **[!UICONTROL Read Service]** 및 **[!UICONTROL Write Service]** 드롭다운 목록에서 **[!UICONTROL get]** 및 **[!UICONTROL insert]**&#x200B;을 선택합니다. 읽기 서비스의 인수를 지정하고 **[!UICONTROL Done]**&#x200B;을 누릅니다.
...
[Automated forms conversion 서비스를 사용하여 ](convert-existing-forms-to-adaptive-forms.md) [연락처 양식](#sample-adaptive-form)을 데이터 바인딩이 있는 응용 양식으로 변환하십시오. 적응형 양식을 생성하는 동안 **[!UICONTROL Generate adaptive form(s) without data bindings]** 확인란을 선택하지 않아야 합니다.

![JSON 바인딩이 있는 적응형 양식](assets/generate_af_with_data_bindings.png)

**[!UICONTROL Forms & Documents]**&#x200B;의 **[!UICONTROL output]** 폴더에서 사용할 수 있는 변환된 **연락처 양식**&#x200B;을 선택하고 **[!UICONTROL Edit]**&#x200B;를 누릅니다. **[!UICONTROL Preview]**&#x200B;을 누르고 적응형 양식 필드에 값을 입력한 다음 **[!UICONTROL Submit]**&#x200B;을 누릅니다.

**crx-repository**&#x200B;에 로그온하고 */content/forms/fp/admin/submit/data*&#x200B;으로 이동하여 JSON 형식으로 제출된 값을 확인합니다. 변환된 **Contact Us** 응용 양식을 제출할 때 JSON 형식의 샘플 데이터는 다음과 같습니다.
...
응용 양식 데이터를 데이터베이스에 제출하는 워크플로우 모델을 생성하려면 다음 단계를 수행하십시오.

1. 워크플로우 모델 콘솔을 엽니다. 기본 URL은 `https://server:port/libs/cq/workflow/admin/console/content/models.html/etc/workflow/models`.

1. **[!UICONTROL Create]**, **[!UICONTROL Create Model]**&#x200B;을 차례로 선택합니다. **[!UICONTROL Add Workflow Model]** 대화 상자가 나타납니다.

1. **[!UICONTROL Title]** 및 **[!UICONTROL Name]**(선택 사항)을 입력합니다. 예: **workflow_json_submit**. **[!UICONTROL Done]**&#x200B;을 눌러 모델을 생성합니다.

1. 워크플로우 모델을 선택하고 **[!UICONTROL Edit]**&#x200B;을 눌러 모델을 편집 모드로 엽니다. +를 누르고 워크플로우 모델에 **[!UICONTROL Invoke Form Data Model Service]** 단계를 추가합니다.
...
테이블에 정의된 기본 키를 기반으로 MYSQL 데이터베이스의 값을 미리 채우기 위해 응용 양식을 구성하려면 다음 단계를 수행하십시오(이 경우 전자 메일).

1. 응용 양식의 **전자 메일** 필드를 누르고 ![규칙 편집](assets/edit-rules.png)을 누릅니다.

1. **[!UICONTROL Create]**&#x200B;을 누르고 **[!UICONTROL When]** 섹션의 **[!UICONTROL Select State]** 드롭다운 목록에서 **[!UICONTROL is changed]**&#x200B;을 선택합니다.

1. **[!UICONTROL Then]** 섹션에서 **[!UICONTROL Invoke Service]** 및 **get**&#x200B;을 이 문서의 이전 섹션에 만든 양식 데이터 모델의 서비스로 선택합니다.

1. **[!UICONTROL Input]** 섹션에서 **전자 메일**&#x200B;을 선택하고 양식 데이터 모델의 나머지 세 필드, **이름**, **전화 번호** 및 **발행물 설명**&#x200B;을 **[!UICONTROL Output]** 섹션에서 선택합니다. **[!UICONTROL Done]**&#x200B;을 눌러 설정을 저장합니다.

   ![이메일 자동 완성 설정 구성](assets/email_prefill_settings.png)

   따라서 MYSQL 데이터베이스의 기존 전자 메일 항목을 기준으로 응용 양식의 **[!UICONTROL Preview]** 모드에서 나머지 세 필드에 대한 값을 미리 입력할 수 있습니다. 예를 들어 **전자 메일** 필드(이 아티클의 기존 데이터 기반)에 aya.tan@xyz.com을 지정하고 필드 밖으로 탭하면 나머지 세 필드, **이름**, **전화 번호** 및 **문제 description**&#x200B;은 적응형 양식에 자동으로 표시됩니다.](#prepare-data-for-form-model)[
