...
seo-description: 자동화된 양식 전환 서비스에 대한 FAQ
...
1. **자동화된 양식 전환 서비스는 어떤 AEM Forms 버전을 지원합니까?**
   <p>자동화된 양식 전환 서비스는 AEM 6.4 양식 및 AEM 6.5 양식을 지원합니다. JEE의 OSGi 및 AEM 양식에서 두 AEM Forms과 함께 작동합니다. 서비스를 사용하려면 AEM 작성자 인스턴스 상단에 최신 AEM Forms 추가 기능 패키지가 필요합니다. For detailed instructions, see <a href="configure-service.md">Configure the Automated Forms Conversion</a> service.</p> 
    <br>

1. **서비스를 자체적으로 설치할 수 있습니까?**
   <p>Adobe는 새로운 데이터 세트를 사용하여 자동 양식 변환 서비스의 AI 및 ML 알고리즘을 정기적으로 교육하여 전환율 정확도를 높입니다. 업데이트된 알고리즘은 Adobe Cloud에서 실행되는 전환 서비스에 주기적으로 배포됩니다. 서비스의 모든 고객은 업데이트된 알고리즘으로 혜택을 받습니다. 따라서 클라우드 기반의 중앙 배포는 향상된 기능을 지속적으로 학습하고 모든 고객에게 전달하는 자동화된 양식 전환 서비스에 가장 적합합니다.</p> 
    <p>서비스는 빈 양식을 응용 양식으로 변환합니다. 서비스는 채워진 양식과 채워진 양식의 데이터 추출을 지원하지 않습니다. 양식을 서비스로 변환하기 전에 채워진 양식에서 데이터를 제거하고 양식에서 독점 정보를 제거하거나 허용</p> <br>
...
1. **이 서비스는 스캔한 종이 양식과 컬러 양식을 적응형 양식으로 변환할 수 있습니까?**
   <p>서비스는 PDF forms을 응용 양식으로 변환할 수 있습니다. 서비스가 스캔하거나 채워진 양식을 지원하지 않습니다. 기타 제한 사항은 <a href="known-issues.md">알려진 문제</a> 문서를 참조하십시오.</p> <br>
...
1. **서비스가 XDP 기반 양식에서 JavaScript를 자동으로 추출하여 해당 적응형 양식에 적용합니까?**
   <p>서비스는 XFA 기반 양식 또는 Acro Forms의 스크립트를 해당 적응형 양식 규칙으로 자동 변환하지 않습니다. 양식 작성자(양식 작성자)는 <a href="https://helpx.adobe.com/experience-manager/6-5/forms/using/rule-editor.html">규칙 편집기를</a> 사용하여 적응형 양식에 인터랙티브한 요소를 추가할 수 있습니다.</p> <br>

1. **일부 양식 개체가 응용 양식 구성 요소로 올바르게 변환되지 않습니다. 문제 해결 방법**
   <p>자동화된 양식 전환 서비스는 다양한 양식에 대해 교육됩니다. 그러나 AI/ML 기반의 애플리케이션은 교육 데이터와 패턴으로 제한됩니다. 다양한 필드 유형, 레이아웃, 패턴 및 상황에 따라 사람의 인사이트를 구별할 수 있지만 자동화된 인식으로 인식하기는 어려울 수 있습니다. 서비스가 이러한 개체를 식별하지 못하거나 잘못 인식할 수 있습니다. 검토 <a href="review-correct-ui-edited.md" target="_blank">및 수정</a> 편집기를 사용하여 입력 양식의 익숙한 종이 양식 레이아웃에서 필요한 사항을 수정할 수 있습니다.</p> <br/>
...
서비스는 비어 있거나 채워지지 않은 양식만 지원합니다. 채워진 양식 또는 양식에 PII(개인 식별 정보)를 업로드하지 마십시오. 또한 소스 양식에서 미리 채워진 데이터 및 화이트 라벨 PII, 기밀 및 독점적 정보를 제거할 수 있습니다. <br/>
