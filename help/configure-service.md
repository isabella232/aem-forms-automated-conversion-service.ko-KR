...
### AEM Forms Add-on 패키지 {#downloadaemformsaddon} 다운로드 및 설치
...
커넥터 패키지는 AFC-2020.03.1 릴리스에서 제공되는 [자동 감지 논리 섹션](convert-existing-forms-to-adaptive-forms.md#run-the-conversion) 기능에 대한 빠른 액세스 및 개선 사항을 제공합니다. AFC-2020.03.1으로 전달되는 기능과 개선 사항이 필요하지 않은 경우 패키지를 설치하지 마십시오. AEM 패키지 공유](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq650/featurepack/AFCS-Connector-2020.03.1)에서 커넥터 패키지를 다운로드할 수 있습니다.[


### 사용자 정의 테마 및 템플릿 만들기 {#referencepackage}

[프로덕션 모드](https://helpx.adobe.com/experience-manager/6-5/sites/administering/using/production-ready.html)(nosamplecontent runmode)에서 AEM을 시작하는 경우 참조 패키지가 설치되지 않습니다. 참조 패키지에는 샘플 테마와 템플릿이 포함되어 있습니다. automated forms conversion 서비스를 사용하려면 PDF forms을 적응형 양식으로 변환하려면 적어도 하나의 테마와 하나의 템플릿이 필요합니다. 사용자 정의 테마와 템플릿을 직접 만들고 [서비스 구성](#configure-the-cloud-service)을 지정하여 서비스를 사용하기 전에 사용자 정의 템플릿과 테마를 사용합니다.
...
* **관리자**:관리자는 조직의 Adobe 소프트웨어 및 서비스를 관리할 책임이 있습니다. 관리자는 Adobe Cloud에서 실행되는 Automated forms conversion 서비스에 연결할 수 있도록 조직의 개발자에게 액세스 권한을 부여합니다. 관리자가 조직에 대해 프로비저닝되면 관리자는 **[!UICONTROL 'You now have administrator rights to manage Adobe software and services for your organization']** 제목이 있는 이메일을 수신합니다. 관리자의 경우, 앞서 언급한 제목이 있는 전자 메일이 수신되었는지 확인하고 [조직 개발자에게 액세스 권한 부여](#adduseranddevs)로 진행하십시오.

![관리자 액세스 권한 부여 이메일](assets/admin-console-adobe-io-access-grantedx75.png)

* **개발자**:개발자는 로컬 AEM Forms 작성자 인스턴스를 Adobe Cloud에서 실행되는 Automated forms conversion 서비스에 연결합니다. 관리자가 Automated forms conversion 서비스에 연결할 수 있는 권한을 개발자에게 부여하면, 이제 귀하 조직에 대한 Adobe API 통합을 관리하기 위한 개발자 액세스 권한이 있는 이메일이 개발자에게 전송됩니다. 개발자인 경우 전술한 제목이 있는 전자 메일이 수신되었는지 확인하고 [Adobe Cloud의 Automated forms conversion 서비스에 로컬 AEM 인스턴스를 연결합니다.](#connectafcadobeio)
...
관리자가 개발자를 Adobe I/O 프로파일에 추가하면 개발자에게 이메일을 통해 알립니다. 이메일을 받은 후 개발자는 Adobe Cloud](#connectafcadobeio)에서 Automated forms conversion 서비스를 사용하여 로컬 AEM Forms 인스턴스 연결을 진행할 수 있습니다.[
...
#### Adobe 개발자 콘솔 {#createintegration}에 서비스 API 구성
...
AEM 인스턴스를 전환 서비스에 연결하는 클라우드 서비스 구성을 만듭니다. 또한 전환용 템플릿, 테마 및 양식 조각을 지정할 수 있습니다. 각 양식 세트에 대해 별도의 클라우드 서비스 구성을 만들 수 있습니다. 예를 들어 영업 부서 양식에 대해 별도의 구성을 지정하고 고객 지원 양식에 대해 별도의 구성을 설정할 수 있습니다. 클라우드 서비스 구성을 만들려면 다음 단계를 수행하십시오.
...
1. **[!UICONTROL Global]** 폴더를 누르고 **[!UICONTROL Create]**&#x200B;을 누릅니다. automated forms conversion 구성을 만들 페이지가 나타납니다. 구성은 글로벌 폴더에 생성됩니다. 이미 존재하는 다른 폴더에 구성을 만들거나 구성에 대한 새 폴더를 만들 수도 있습니다.
...
1. **[!UICONTROL Create Automated Forms Conversion Configuration]** 페이지의 **[!UICONTROL Advanced]** 탭에서 다음 필드의 값을 지정합니다.
   ...
   <tr>
   <td>Analytics 활성화</td>
   <td>변환된 모든 양식에서 Adobe Analytics을 활성화하려면 옵션을 선택합니다. 이 옵션을 사용하기 전에 AEM Forms 인스턴스에 대해 Adobe Analytics이 활성화되어 있는지 확인합니다.</td>
   </tr>
   </tbody>
   </table>
